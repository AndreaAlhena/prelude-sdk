...
describe('VerificationService', function () {
    ...
    describe('getVerificationStatus method', function () {
        it('can get verification status successfully', function () {
            // Arrange
            $httpClientMock = test()->createMock(HttpClient::class);
            $service = new VerificationService($httpClientMock);
            ...
            $httpClientMock
                ->expects(test()->once())
                ->method('get')
                ->with(Config::ENDPOINT_VERIFICATION . '/' . $verificationId)
                ->willReturn($mockResponse);
            
            // Act
            $result = $service->getVerificationStatus($verificationId);
        ...
        it('throws ApiException when verification not found (400 response)', function () {
            // Arrange
            $httpClientMock = test()->createMock(HttpClient::class);
            $service = new VerificationService($httpClientMock);
            ...
            // Act & Assert
            try {
                $service->getVerificationStatus($verificationId);
                expect(false)->toBeTrue('Expected ApiException to be thrown');
            } catch (ApiException $e) {
                expect($e->getMessage())->toBe('Verification not found.');
                expect($e->getCode())->toBe(400);
                expect($e->getResponseData())->toBe([
                    'code' => 'verification_not_found',
                    'message' => 'Verification not found.',
    ...
    describe('cancelVerification method', function () {
        it('can cancel verification successfully', function () {
            // Arrange
            $httpClientMock = test()->createMock(HttpClient::class);
            $service = new VerificationService($httpClientMock);
            ...
            // Act
            $result = $service->cancelVerification($verificationId);
        ...
        it('throws ApiException when verification cannot be cancelled (400 response)', function () {
            // Arrange
            $httpClientMock = test()->createMock(HttpClient::class);
            $service = new VerificationService($httpClientMock);
            ...
            // Create an ApiException with the specific error structure from the API
            $apiException = new ApiException(
                'Verification cannot be cancelled.',
                400,
                null,
                [
                    'code' => 'verification_not_cancellable',
                    'message' => 'Verification cannot be cancelled.',
                    'type' => 'bad_request',
                    'request_id' => '3d19215e-2991-4a05-a41a-527314e6ff6a'
                ]
            );
            ...
            // Act & Assert
            try {
                $service->cancelVerification($verificationId);
                expect(false)->toBeTrue('Expected ApiException to be thrown');
            } catch (ApiException $e) {
                expect($e->getMessage())->toBe('Verification cannot be cancelled.');
                expect($e->getCode())->toBe(400);
                expect($e->getResponseData())->toBe([
                    'code' => 'verification_not_cancellable',
                    'message' => 'Verification cannot be cancelled.',
